...
class Userlogin extends Component
{
     ...
     public function logon(){
          try{
               $user=UsersTable::where(['email'=>$this->email])->first();
             ...
             if($user and $user->account_status=='verified' and  Hash::check($this->password, $user->password)){
                 ...
                 #update authcode
                 $updateAuth=UsersTable::where(['email'=>$this->email])->update([
            ...
            elseif($user and  $user->account_status=='unverified'){
                  session()->flash('error','Please check your email and verify your account');
              }
            elseif($user and $user->account_status=='blocked'){
                session()->flash('error','Opps sorry your account have being block please contact admin');
            }
