...
class Checkoutpay extends Controller
{
    ...
    public function CheckOutListing(Request $request){
        ...
        $addpayment=PaymentTable::create([
        ...
        #api key
        $apikey=AdminTable::where('account_type','master')->first();
        $key=$apikey->apikey;
               ...
               //Coinbase Commerce Api
                ...
                curl_setopt_array($curl, [
                  ...
                  CURLOPT_POSTFIELDS => "{
                      ...
                      \"currency\":\"USD\"},
                      \"name\":\"Chaincity Real Esate and property listing\",
                ...
                if ($err) {
                  echo "cURL Error #:" . $err;
                    ...
                    $reservationUpdate=SavedListingTable::where([
                     ...
                     #save reservation
                     $reservationSelect=SavedListingTable::where(['payment_id'=>$txncode])->get();
                    ...
                    foreach($reservationSelect as $reservations){
                        ...
                        $insertReservation=ReservationTable::create($reserveData);
                    

                        #update payment with ordercode...
                        $updateOrder=PaymentTable::where(['transaction_id'=>$returnResponse])->update([
                            'payment_id'=>$output
                        ]);
                        if($updateOrder){
    ...
    public function success($id){
          ...
          #save reservation
          $reservationSelect=SavedListingTable::where(['payment_id'=>$id])->get();

          foreach($reservationSelect as $reservations){
              
             #make property unavaliable 
             $reserveProperty=PropertyTable::where('id',$reservations->property_id)->update([
               'property_process->status'=>'booked'
             ]);

             #get agent Id
             $agentId=PropertyTable::where('id',$reservations->property_id)->first();

            #insert message
            $sendMessage=Messaging::insert([
            ...
            $agentpayment=Earnings::insert([
               'user_id'=>$agentId->agent_id,
               'amount'=>(($agentPercent->agent_percent/100)*$reservations->saved_description['payable']),
               'type'=>'cashout',
               'status'=>'pending',
               'created_at'=>Carbon::now()
            ]);
          ...
          $reservationUpdate=SavedListingTable::where(['payment_id'=>$id])
           ->orWhere(['property_id'=>$reservations->property_id])->delete();

          #update payment with paymentID...
          $updateOrder=PaymentTable::where(['transaction_id'=>$id])->update([
            'transaction_status'=>'completed'
          ]);

            $agentDetails=UsersTable::where('id',$agentId->agent_id)->first();
